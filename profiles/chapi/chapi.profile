 ...
 */
function chapi_form_install_configure_form_alter(&$form, $form_state) {
  // Pre-populate the site name with the server name.
  $form['site_information']['site_name']['#default_value'] = $_SERVER['SERVER_NAME'];
  $form['#submit'][] = 'chapi_form_submit';
}


function chapi_form_submit($form, &$form_state) {
  // Import database chapi file.
  $chapi_file = dirname(__FILE__) . '/chapi.sql';
  $success = import_chapi($chapi_file);
...
/**
 * Imports a database chapi file.
 *
 * @see demo_reset().
 */
function import_chapi($filename) {
  ...
  if (!file_exists($filename) || !($fp = fopen($filename, 'r'))) {
    drupal_set_message(t('Unable to open chapi file %filename.', array('%filename' => $filename)), 'error');
    return FALSE;
  }
